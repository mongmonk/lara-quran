        ...
        @include('sidenav')
            <div class="post">
                ...
                @foreach ($data as $ruku)
                    @php
                        $number = app('quran')->numConverter($ruku['ayat']);
                        $urutan = $ruku['numList'];
                    @endphp
                    <div class='ayahAudio{{ $urutan }}'>
                        <p class='text-right arab'>{!! $ruku['ar'] !!} {!! $number !!}</p>
                        <p><span class='btn btn-sm btn-secondary'>ID Translation</span> {!! $ruku['id'] !!}</p>
                        <p><span class='btn btn-sm btn-secondary'>EN Translation</span> {!! $ruku['en'] !!}</p>
                        <div class="line"></div>
                    </div>
                @endforeach
            </div>
    ...
    <div class="bg-light fixed-bottom py-2 footer">
        <div class="container">
            <div class="d-flex justify-content-center mb-2">
                ...
                <button type="button" class="mx-1 btn btn-danger" target="popup" onclick="window.open('https://www.pinterest.com/pin/create/button/?description={{ url()->current() }}&media={{ asset('inc/images/alquran.png') }}','popup','width=700,height=500'); return false;">
            ...
            <div class="row justify-content-center" id="surahConfigurator">
              <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                <select id="surahSelector" class="form-control mb-2" style="height: 54px !important;" onchange="if (this.value) window.location.href=this.value">
                    @foreach ($meta as $key => $rukuItem)
                        @if (request()->segment(2) == $rukuItem['ruku'])
                            <option value='/ruku/{{ $rukuItem['ruku'] }}' selected>ركوع  ﴿{{ app('quran')->numConverter($rukuItem['ruku']) }}﴾ , {{ $meta[$rukuItem['ruku']-1]['surahAr'] ?? '' }}</option>
                        @else
                            <option value='/ruku/{{ $rukuItem['ruku'] }}'>ركوع  ﴿{{ app('quran')->numConverter($rukuItem['ruku']) }}﴾ , {{ $meta[$rukuItem['ruku']-1]['surahAr'] ?? '' }}</option>
                        @endif                                
                    @endforeach
                </select>
              </div>
