        ...
        <form action="{{ url()->current() }}">
          <select class="form-control" name="surah" onchange="this.form.submit()">
              @foreach (app('quran')->surahListStatis() as $key => $value)
                  @if ($value['id'] == request()->segment(2))
                      <option value='{{ $value['surahNum'] }}' selected>{!! $value['surahAr'] !!}</option>
                  @else
                      <option value='{{ $value['surahNum'] }}'>{!! $value['surahAr'] !!}</option>
                  @endif                                
              @endforeach
          </select>
        </form>        
