...
<body>
    <div class="wrapper mb-3 pb-5">
        @include('sidenav')
            <div class="post">
                <h1 class='text-center arab text-dark'>Juz {{ $data[0]['juz'] }}</h1>
                ...
                @foreach ($data as $juz)
                    ...
                    <div class='ayahAudio{{ $urutan }}'>
                        ...
                        <p><span class='btn btn-sm btn-secondary'>ID Translation</span> {!! $juz['id'] !!}</p>
                        <p><span class='btn btn-sm btn-secondary'>EN Translation</span> {!! $juz['en'] !!}</p>
                        <div class="line"></div>
                    </div>
                @endforeach
            </div>
    ...
    <div class="bg-light fixed-bottom py-2 footer">
        <div class="container">
                ...
                <select id="juzSelector" class="form-control mb-2" style="height: 54px !important;" onchange="if (this.value) window.location.href=this.value">
                    @foreach (range(1, 30) as $key => $value)
                        @if ($value == request()->segment(2))
                            <option value='/juz/{{ $value }}' selected>Juz {{ $value }}</option>
                        @else
                            <option value='/juz/{{ $value }}'>Juz {{ $value }}</option>
                        @endif                                
                    @endforeach
                </select>
              </div>
            ...
            <div class="text-center text-secondary">&copy;{{ date('Y') }} <a href="{{ url('/') }}">{{ config('app.name') }}</a>. Developed by <a href="mailto:akunpribadiku@gmail.com">Cemonggaul</a></div>
        </div>
    </div>
